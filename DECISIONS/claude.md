...
## 2026-09-01 — CI(Linux)가 처음 돌자 13개가 무너졌다

이 브랜치의 커밋 아홉 개 동안 CI 는 **한 번도 돌지 않았다.** 워크플로가
`push: branches: [main]` 과 `pull_request` 에만 걸려 있어서다. 브랜치에 쌓는
동안에는 아무도 보지 않는다. PR 을 열자 ubuntu 3.11·3.12·3.13 세 판이 모두
같은 자리에서 실패했다.

### 원인 하나

가짜 `subprocess` 로 도는 시험이 **이 PC 에 그 앱이 깔려 있는지에 기대고
있었다.** 명령을 완전 경로로 부르도록 고치면서 `shutil.which` 가 끼어들었는데,
`subprocess.run` 만 가짜였고 `which` 는 진짜였다. `codex`·`claude`·`code` 가
깔린 Windows 에서는 통과하고, 아무것도 없는 CI 에서는 전부 "명령을 찾지
못했다" 로 끝났다.

시험이 보려던 것(인자 구성, 다시 읽어 판정, 남의 항목 보호)과 아무 상관없는
환경 의존이다. `PretendsCommandsExist` 가 `setUp` 에서 한 번에 끊는다. 없는
명령을 일부러 보는 시험은 안에서 다시 덮어쓴다.

### 이번에도 모킹이 가렸다

VS Code 의 `.CMD` 문제와 같은 종류다. 그때는 `subprocess.run` 을 가짜로 바꿔서
"명령이 정말 실행되는가" 를 아무도 보지 않았고, 이번에는 `which` 를 진짜로
두어서 "이 시험이 이 PC 밖에서도 도는가" 를 아무도 보지 않았다.

### 다음부터 로컬에서 걸러낸다

PATH 를 비워 CI 를 흉내 낼 수 있다.

    PATH="<python 폴더>:/c/Windows/System32" python -m unittest discover -s tests

이렇게 하면 `codex`·`claude`·`code`·`gemini` 가 모두 없는 상태가 된다. 고친 뒤
이 방법으로 429 개가 통과하는 것을 확인하고 올렸다.

## 2026-09-01 — 화면을 띄워 보고서야 알았다: 연결 버튼이 잘려 있었다

앱 일곱을 체크박스로 늘어놓았는데 창 크기는 `620x500` 으로 못 박혀 있었다.
필요한 높이는 **648px**. 148px 이 잘렸고, 잘린 자리가 하필 마지막에 놓인
것들이었다. **"4. 연결하기" 제목, 연결 버튼, 진행 막대, 상태 문구.**

버튼이 화면 밖이라 **연결 자체가 불가능하다.** 시험 429개가 모두 통과하는
상태에서 설치 프로그램은 못 쓰는 물건이었다. 화면을 띄워 보지 않았으면
그대로 배포됐다.

### 두 가지를 함께 고쳤다

- **크기를 숫자로 못 박지 않는다.** 위젯을 다 만든 뒤 `_fit_to_contents` 가
  내용에 맞춰 정한다. 화면 높이의 88% 를 상한으로 두어 창 자체가 화면 밖으로
  나가지도 않게 했다. 앞으로 앱이 늘어도 잘리지 않는다.
- **없는 앱은 줄을 차지하지 않는다.** 잠긴 체크박스로 늘어놓으면 목록이 앱
  수만큼 길어진다. 찾은 앱만 체크박스로 두고, 나머지는 회색 한 줄로 이름만
  알린다. 네 줄이 한 줄이 된다. 알리는 것은 그대로다.
...
## 2026-09-03 — 인자가 빠졌을 때 무엇이 빠졌는지 말한다

`cueprecise_status` 를 인자 없이 부르면 이렇게 나왔다.

```
실패: 'video_id'
```

`arguments["video_id"]` 의 KeyError 문구가 그대로 호스트까지 나간 것이다. 이
줄만 보고는 값이 빠진 건지, 그런 이름의 영상이 없는 건지, 코드가 깨진 건지
구분할 수 없다.

**도구를 부르는 쪽이 사람이 아니라 모델이라는 점이 중요하다.** 무엇이 빠졌는지
말해 주면 모델은 스스로 고쳐서 다시 부른다. 안 말해 주면 같은 실패를 반복하거나
사용자에게 알 수 없는 오류를 그대로 전달한다. 이 한 번을 벌기 위한 검사다.

### 스키마를 이미 갖고 있었다

`TOOLS` 의 `inputSchema` 에 `required` 와 타입이 전부 적혀 있다. 도구마다 검사를
손으로 쓰지 않고 `dispatch` 앞에 `validate_arguments()` 한 번을 둔다. 도구를
새로 넣어도 스키마만 제대로 쓰면 검사가 따라온다.

```
cueprecise_status 에 필요한 값이 빠졌다: video_id
cueprecise_excerpt 의 start 는 숫자여야 한다.
```

빠진 것이 여럿이면 한 줄에 모아 알린다. 하나씩 알리면 왕복이 그만큼 늘어난다.

### bool 을 따로 걸러낸다

파이썬에서 `True` 는 `int` 의 인스턴스다. `isinstance(value, int)` 로만 보면
`max_entries: true` 가 `1` 로 조용히 통과한다. 반대 방향(숫자를 참/거짓 자리에)
도 같이 막았다.

### 비용

`validate_arguments()` 는 도구 호출 1건당 한 번, 1.71 us 다. 그 뒤에 이어지는
것이 파일 읽기나 SQLite 질의라 견줄 크기가 아니다. `tools/list` 는 이 경로를
지나지 않아 0.70 us 그대로다.

453 개 통과.

## 2026-09-04 · 설치 경로에서 API 키가 새는 곳 넷을 막았다

전사와 조회 경로는 손대지 않았다. 먼저 확인한 것이 그것이다 — `pipeline.status`
와 `usage.get_usage` 의 반환값, 설치 결과 dict 네 곳(`api_key_configured` 는 값이
아니라 불리언), `cueprecise_cli` 가 찍는 JSON 어디에도 키가 없다. **키가 MCP 도구
응답을 타고 LLM 으로 나가는 경로는 없다.** 그것이 있었다면 다른 무엇보다 먼저
고쳤을 것이다.
...
- **오류 메시지 마스킹.** `configuration.mask_secrets` 를 만들고 `_confirm`,
  `remove`, `_run` 의 예외, `probe_mcp`, winget 실패 네 자리에 걸었다.
- **백업에서 키 제거.** `backup_file` 이 사본에서 `env.GEMINI_API_KEY` 를 지우고,
  설정 파일 하나당 최근 3개(`BACKUP_KEEP`)만 남긴다.
- **`.tmp` 정리.** `write_config` 의 `replace` 가 실패하면 키를 담은 임시 파일을
  지우고 예외를 다시 던진다.
- **`--api-key-file` 과 `--api-key -`.** 명령줄에 키를 적지 않고 등록하는 길.

### 왜 마스킹을 자르기 전에 하는가

원래 코드는 `(stderr or stdout).strip()[-300:]` 이었다. 먼저 자르면 잘림 경계가
키 한가운데를 지날 때 **뒤 절반이 그대로 남는다.** 그 조각은 `AIza` 로 시작하지
않으므로 생김새 그물에도 걸리지 않는다. 그래서 `mask_secrets(...)[-300:]` 로
순서를 뒤집었다.

`test_masking_runs_before_truncation` 은 처음에 패딩을 400 자로 잡았다가 다시
썼다. 그 길이에서는 키가 통째로 잘려 나가 **마스킹을 꺼도 통과했다.** 지키는 것이
없는 시험이었다. 경계가 키를 관통하도록 320 자로 맞췄다. 이번에 넣은 방어는 전부
한 번씩 무력화해 보고 시험이 무너지는 것을 확인했다 — 6 건, 4 건이 각각 무너졌다.

### 받아들이지 않은 것 — 생김새 그물만으로 막기

`AIza[0-9A-Za-z_-]{30,}` 하나로 끝낼 수 있어 보인다. 그럴 수 없다. Workbrain 에
남아 있는 조사대로 **2026 년부터 발급되는 Google Auth 키는 `AIza` 로 시작하지
않는다.** 그래서 `mask_secrets` 는 값을 아는 자리에서 값 자체를 받아 지우고,
`_secrets_in` 이 우리가 만든 명령줄에서 값을 되찾아 온다. 생김새는 마지막 보루다.

### 새로 찾은 누출 — `TimeoutExpired`

계획에 없던 것이다. `subprocess.TimeoutExpired.__str__` 은 자기 문자열에 **명령줄
전체**를 담는다. `codex mcp add --env GEMINI_API_KEY=<키> ...` 가 통째로 들어
있다. 시간 초과 한 번에 키가 화면으로 나온다. `_run` 의 `except` 에서 지운다.

### 백업을 거를 때 지킨 것

백업은 복구 수단이다. 거르다 깨뜨리면 복구가 사라진다. 그래서 JSON 으로 읽히지
않는 파일(Codex 의 TOML)은 **해석하지 않고 그대로 복사**하고 개수 제한에만 맡긴다.
`CONTRACT` 이 금지한 TOML 쓰기를 우회로 들여오지 않기 위해서이기도 하다. 다시
쓰기가 실패하면 키가 든 사본이 남는데 그것이 막으려던 상태이므로, 그때는 사본을
지우고 백업 없이 진행한다.

### 소유권 — 남의 파일을 고쳤다

`src/configuration.py`, `src/installer_support.py`, `src/cueprecise_cli.py`,
`tests/test_cli.py` 는 표에서 codex 소유다. 이 파일 머리의 위임(2026-08-30)과
1302·1580 줄의 선례를 근거로 고쳤다.

**`tests/test_naming.py` 는 사람 소유다.** 위임 범위 밖이다. 고칠 수밖에 없었던
이유는 그 파일의 `test_config_is_backed_up_before_being_changed` 가 "백업에 키가
들어 있다" 를 단언하고 있었기 때문이다. 백업에서 키를 지우는 것과 양립하지 않는다.
지우지 않고 **단언만 바꿨다** — 되돌리는 데 필요한 것(`command`, `args`, `env.PATH`)은
그대로 요구하고 비밀값만 빠지게 했으며, 키가 사본에 없다는 단언을 더했다. 이 시험이
무엇을 지키는지는 그대로다. 소유자의 판단을 기다린다.

### 남긴 것

argv 노출(1580 줄에서 받아들인 대가)과 설정 파일 평문 저장은 그대로다. 둘 다
"서버가 키를 스스로 읽고 클라이언트 설정에는 키를 넣지 않는" 구조로 가야 사라진다.
그것은 읽는 자리 6 곳(`transcribe.py:258`, `pipeline.py:1267`·`1504`,
`usage.py:257`, `mcp_server.py:657`, `cueprecise_cli.py:97`·`129`)을 하나로 모으고,
`setup_file_client` 의 환경변수 물려받기에서 `GEMINI_API_KEY` 만 빼고, 7 개
클라이언트를 도는 이관을 새로 쓰는 일이다. `migrate` 는 지금 Claude Desktop 하나만
...
## 2026-09-18 · 106분 강연(1121문장) 번역에서 MCP 왕복이 예상의 2배

**측정:** `tr-CUpw--ck`. 전체 호출 약 50회 중 20회가 한 줄짜리 왕복이었다.
원인 3가지 전부 코드로 확인:

1. `_translate_batch_for` 가 **연속 구간**만 묶었다. 저장된 줄을 만나면 즉시
   끊어, 거절돼 혼자 남은 한 줄이 다음 라운드에서 그 한 줄만 담긴 패킷이 됐다.
2. 거절 사유 상위 2종(`/k 숨 지점이 증가 순서가 아니다`, `한글 비율이 30%
   미만이다`)이 translate 패킷 `instructions` 에 규칙으로 없었다 — 검증기만
   알고 번역자는 몰랐다.
3. `pace` 표시 23건. 패킷은 문장 전체 `char_budget` 만 주는데 검사
   (`_pace_exceeded`)는 `/k` 로 나뉜 구간 단위로 한다.

**고친 것.**
- `_translate_batch_for`(src/subtitle.py): 저장된 줄만 건너뛰고 미저장 줄을
  `BATCH_CHARS` 까지 모은다(단일 줄 패킷 제거). 건너뛴 구간은
  `_render_translate_body` 가 "-- N~M번 이미 번역됨(생략) --" 로 패킷에 남겨
  번역자가 앞뒤가 이어진다고 오해하지 않게 한다.
- `_packet_translate` instructions 에 하드 규칙 3개 추가: /k·/+n 오름차순,
  기호·숫자뿐인 원문도 번역문 한글 비율 30% 이상, /k 구간별 초당 약 13자
  (`READ_CPS`) 한도. 문장 늘리지 않고 짧게.

**검토에서 바뀐 판단:** 처음엔 `/k` 구간별 정확한 글자상한을 패킷 표에 넣을까
했으나, 숨 지점 후보 중 실제로 어디서 끊을지는 번역자가 정하므로 사전 계산이
무의미하다 — 텍스트 힌트로 충분하다고 판단해 패킷 줄 형식은 건드리지 않았다.
병합(`<번호>|=`) 로직은 배치가 아니라 `indexed.by_no`/`state["lines"]` 를 직접
봐서, 배치가 불연속이어도 영향이 없음을 코드로 확인했다(수정 불필요).
`BATCH_BREAK_GAP_SECS`(2.0s) 자연 경계 판정은 건너뛴 구간으로 생긴 큰 시간차도
"자연스러운 경계"로 취급하지만, `total_chars >= BATCH_CHARS*0.5` 일 때만
멈추므로 작은 배치가 거기서 또 끊기지는 않는다 — 그대로 둠.

**CONTRACT.md 16절은 고치지 않았다.** "장면 단위 묶음, 줄 단위 텍스트"라는
명세 수준에서는 안 바뀐다 — 묶음 구성 알고리즘의 내부 최적화다.

**테스트:** `TranslateBatchGapTests` 2개 추가(중간에 구멍 난 배치가 다음
