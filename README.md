...
완료된 청크는 다시 부르지 않는다. **설정을 바꾸지 않고 재실행하면** 저장된
응답을 그대로 다시 읽어 같은 판정이 나온다 (호출 0). 그래서 언어를 반드시
바꿔줘야 한다. 언어를 바꾸면 청크마다 새 호출이 나간다 — 저장된 응답은
요청 언어가 다르면 재사용하지 않는다. 타임스탬프가 조용히 어긋나는 것보다
호출 한 번이 낫다.
