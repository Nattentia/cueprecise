...
def request_raw(audio: str, langs: str | None, *,
                api_key: str | None = None) -> tuple[dict[str, Any], list[str] | None]:
    """Gemini 를 호출해 응답 원문을 그대로 돌려준다. 검증하지 않는다."""
    if not os.path.isfile(audio):
        raise FileNotFoundError(f"오디오 파일이 없습니다: {audio}")
    provider_key = api_key or os.environ.get("GEMINI_API_KEY")
    if not provider_key:
        raise EnvironmentError("GEMINI_API_KEY 환경변수가 설정되지 않았습니다.")

    from google import genai  # SDK 는 실제 호출 경로에서만 필요하다.

    client = genai.Client(api_key=provider_key)
...
def transcribe(audio: str, langs: str | None, raw_path: Path | None = None,
    ...
    """
    raw, codes = request_raw(audio, langs, api_key=api_key)
