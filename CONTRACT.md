...
마지막 항목이 핵심이다. 초기 비교 기준으로 사용한 전사 도구는
...
연결한다. MCP 도구 이름은 `cueprecise_` 접두사를 쓴다.

`mcp_server.py` 는 스펙 `2026-07-28`(악수 없이 요청마다 판을 싣는 방식)과 그
이전 악수 방식을 **둘 다 받는다.** `handle()` 은 요청 사이에 아무 상태도 두지
않는 순수 함수로 유지한다. 이것이 신식의 전제이고, 세션을 두는 순간 깨진다.
`server/discover` 는 지원 판과 정체를 알리고, `initialize` 는 그대로 받는다.
`_meta` 에 우리가 모르는 판이 실려 오면 `-32022` 와 지원 목록으로 거절하되,
판이 실려 있지 않으면 따지지 않는다. 구식 요청에는 그 자리가 없기 때문이다.

`mcp_server.py`는 최소 다음 동작을 제공한다.
