...
> 동작합니다. `v0.2.9`부터는 다른 AI 앱용 설치 파일도 서명되지 않은 실행 파일 대신 같은
...
영상 자막이나 YouTube 자동 번역이 부정확할 때, AI 앱이 같은 근거로 자막을 만듭니다.
CuePrecise가 Gemini 전사를 YouTube 자막·화면 글자와 대조하고 전문용어부터 확정한 뒤
(예: `by torch → PyTorch`, `combine JS → ConvNetJS`), AI 앱이 강연을 작은 묶음으로 나눠
번역합니다. 숫자·용어·읽기 속도·빠진 줄을 자동으로 검사하고, 걸린 줄만 다시 봅니다.

결과는 브라우저의 로컬 뷰어로 열립니다. YouTube 영상에 번역 자막이 겹쳐 나오고, 검색할
수 있는 스크립트, 켜고 끌 수 있는 짧은 용어 해설, 품질 보고를 함께 봅니다. 번역은 사용
중인 AI 앱이 하므로 Gemini를 추가로 호출하지 않습니다.
...
현재 `v0.2.9` 배포 파일은 서명되지 않은 시험판입니다. Windows에서 알 수 없는 게시자
