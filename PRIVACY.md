...
ytx는 별도의 운영 서버나 사용자 계정을 제공하지 않는다. 프로젝트 운영자는 사용자의
API 키, 영상, 대본 또는 Claude 대화를 수집하지 않는다.

## 외부로 전송되는 정보

ytx는 사용자가 영상 분석을 요청하거나 설치 화면에서 관련 기능을 선택할 때 다음 외부
서비스와 통신한다.
...
이 통신은 사용자가 설치 또는 분석을 요청한 경우에만 발생한다. ytx는 광고, 분석 추적,
원격 측정(telemetry) 또는 자체 서버로의 정보 전송 기능을 포함하지 않는다.

## 컴퓨터에 저장되는 정보

- Gemini API 키는 Claude Desktop이 ytx를 실행할 수 있도록 사용자의 로컬 Claude
  Desktop 설정 파일에 평문으로 저장된다. 저장소나 프로젝트 운영자에게 전송되지 않는다.
- 분석 결과와 내려받은 자료는 기본적으로 사용자 홈 폴더의 `.ytx/data` 아래에 저장된다.
...
Windows 설치 프로그램은 사용자 계정의 `%LOCALAPPDATA%\Programs\ytx`에 프로그램을
설치하고 시작 메뉴 바로가기를 만든다. 연결 버튼을 누르면 Claude Desktop 설정에 `ytx`
항목을 추가하고, 필요한 경우 WinGet으로 FFmpeg를 설치한다.

Windows 설정의 **앱 > 설치된 앱 > ytx > 제거** 또는 시작 메뉴의 **ytx 제거**를 사용하면
프로그램과 Claude Desktop의 `ytx` 연결 항목을 제거한다. 다른 MCP 설정, FFmpeg, Claude
설정 백업 및 `.ytx/data`의 분석 자료는 자동으로 삭제하지 않는다. 사용자는 보존 여부를
확인한 뒤 이 자료를 직접 삭제할 수 있다.
